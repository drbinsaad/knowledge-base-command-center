...
## Book covers are missing or too small
...
If controls clip or the software keyboard covers content, record the iPhone model, iOS version, Obsidian version, portrait/landscape orientation, Dynamic Type setting, and a sanitized screenshot. The [0.19.0 physical-device record](release-evidence/0.19.0-iphone.md) is waived and unverified; its supplemental Mac emulation is not a physical-iPhone Pass, so do not assume the complete matrix passed.
