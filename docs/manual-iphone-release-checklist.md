...
- Prepare a synthetic multi-base portfolio with at least two source bases, more than 50 preview rows in one category, an empty Library, an empty heading, a placeholder, an unplaced subject, one unavailable template, and one unavailable destination folder. Keep a second-vault copy for the cross-vault gate cases.
