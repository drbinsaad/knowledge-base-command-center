...
The plugin has no intended analytics, telemetry, advertising, account, or payment behavior. It makes no network request by default. The unreleased Library-cover implementation can load HTTPS images only after an explicit device-local opt-in. Ordinary organization changes plugin data only. The [Library display and privacy proposal](docs/LIBRARY_DISPLAY_AND_PRIVACY.md) is pending independent and public review before release; local implementation is not evidence that review is complete.
...
Library display profiles contain property names and visual choices, not the property values or cover URLs. Local covers must resolve to existing vault raster images. After permission, remote images use browser image requests: hosts receive the requested URL and IP address; referrers are suppressed, but ordinary browser cookies, cache, redirects, and host tracking can apply. There is no guarantee that a remote server returns the expected content type. Permission is never read from synced data, imports, or recovery files. Revocation removes KBCC's current external image sources and prevents new loads; it cannot undo requests already sent. If clearing permission fails in App-local storage, images stay blocked in the current session and the user must check again after restarting.
...
The fifth, vault-specific App-local value stores only the versioned external-image permission boolean. It applies to all Libraries in this vault on this device and does not contain cover URLs, paths, note content, or host history. **Clear device-local data** also clears this permission; it does not change another vault's or device's permission or clear the browser's cookies or image cache. See [Local data](docs/LOCAL_DATA.md).
