...
Knowledge Base Command Center runs inside the current Obsidian vault. It intentionally enumerates Markdown paths and cached metadata to build indexes and enumerates vault entries for folder and in-vault JSON pickers. Content reads are targeted to a selected template or import, to the note explicitly selected for Quick append inside Obsidian's atomic process operation, to an explicit attachment destination note, and to separately disclosed ENT canonical workflows. The explicit Attach file command reads only the external files selected by the user (up to 20 per action) and writes their bytes to validated vault paths; it does not enumerate external files, request direct camera access or capture, intercept paste/drop, or transmit the binary. On mobile, any camera or photo-library source offered by the operating system's file picker remains an explicit user choice.

The plugin has no intended analytics, telemetry, advertising, account, or payment behavior. Version 0.23.0 loads Library covers only from existing vault images and has no online-image enable control or remote-cover request path. Ordinary organization changes plugin data only. The [earlier online-cover proposal](docs/LIBRARY_DISPLAY_AND_PRIVACY.md#deferred-online-cover-proposal) remains deferred and unapproved; exclusion from this release does not complete its independent public privacy review.
...
Library display profiles contain property names and visual choices, not property values or cover URLs. Covers must resolve to existing vault raster images through Obsidian’s resource API; remote references are rejected. Sync, imports, recovery and history cannot enable online covers. Large or animated local files can still consume significant decoded-image memory; visual dimensions are not a resource limit.

Obsidian's App-local storage also holds device-only routes, bounded Undo/Redo and recovery facts, update-announcement history, and—only while a Markdown rename still needs KBCC organization repair—a bounded vault-scoped rename-recovery journal. That temporary journal contains the vault identity plus the old and new vault-relative paths, but never note bodies. A fourth, bounded return-navigation history can contain the vault identity, up to 24 opened-note paths, their originating base and tab, a selected-record path, literal search text entered in KBCC, compact-detail state, and scroll positions. KBCC does not read or copy note bodies into return history, but user-entered search text can itself be sensitive; the history is not synced. **Clear device-local data** removes all five plugin-owned App-local values, including both bounded path-bearing records; treat a device profile containing them as private.
...
The fifth, vault-specific App-local key may hold an inert external-image permission boolean from a private test build. Version 0.23.0 does not consult it to authorize image loading. **Clear device-local data** includes this legacy key in cleanup; a failed cleanup cannot enable online covers in this release. Cleanup does not change another vault’s or device’s data, clear browser cookies/cache, or undo requests previously sent by a private test build. See [Local data](docs/LOCAL_DATA.md).
...
The attachment command can create new binary files and insert their generated Markdown links (embedded or plain, as the user chooses) after an explicit user action. Existing attachments are never moved or deleted. If link insertion fails after the binary writes, the plugin keeps the binaries and reports their vault paths rather than deleting data automatically. If a later file in the same action cannot be copied, the files already copied are kept and linked, and the failure is reported. Attachment contents are excluded from plugin exports and recovery packages.
