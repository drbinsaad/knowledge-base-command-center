...
interface CreateKnowledgeNoteFormContext {
  ...
  /** Optional combined creator for flows that must commit their own filing. */
  createNote?: (value: GenericNoteFormValue) => Promise<TFile>;
  /** Re-resolved immediately before the Markdown file is created. */
...
const PLACEHOLDER_STATUS_LABEL = "No note";
const RECORD_KEYBOARD_SHORTCUTS = "Enter Space M P";

/** Render a non-interactive icon without hiding its meaning from assistive technology. */
function statusIconBadge(parent: HTMLElement, className: string, icon: string, label: string): HTMLElement {
...
/** Compatibility protection must be visible before a write-only control is activated. */
function disableWhenReadOnly(button: HTMLButtonElement, readOnly: boolean, action: string): void {
  if (!readOnly) return;
  const explanation = `${action} is unavailable while organization data is read-only`;
...
function disclosureButton(parent: HTMLElement, collapsed: boolean, label: string, key = label): HTMLButtonElement {
  ...
  button.setAttribute("data-kbcc-focus", `disclosure:${key}`);
  return button;
}
...
export class EntVaultCommandCenterView extends ItemView {
  ...
  private placeholderQueueCache: {
    records: VaultRecord[];
    baseId: string;
    candidateGeneration: number;
    candidateSettingsKey: string;
    ordered: VaultRecord[];
    total: number;
  ...
  async reload(withinOperation = false): Promise<void> {
    const focusBefore = captureViewFocus(this.contentEl);
    const listScrollBefore = this.paneLayout === "wide" ? this.treeEl?.scrollTop ?? 0 : this.workspaceEl?.scrollTop ?? 0;
    const detailScrollBefore = this.paneLayout === "wide" ? this.inspectorEl?.scrollTop ?? 0
      : this.inspectorEl?.querySelector<HTMLElement>(".ent-cc-inspector-body")?.scrollTop ?? 0;
    this.measureAndApplyPaneLayout(false);
    ...
    if (!tabDefinitions(this.plugin.data.settings, this.records, this.plugin.getLibraries())
      ...
      if (this.searchScope === "library") this.searchScope = "current";
    ...
    if (!baseChanged && this.hasGlobalSearch() && this.globalSearchResultKey !== this.globalSearchKey()) {
      this.pendingReturnScroll = { searchKey: this.globalSearchKey(), listScrollTop: listScrollBefore, detailScrollTop: detailScrollBefore };
    }
    ...
    this.render(!baseChanged);
    if (!baseChanged && this.contentEl?.ownerDocument) {
      this.restoreReturnScrollPosition(listScrollBefore, detailScrollBefore);
      restoreViewFocus(this.contentEl, focusBefore);
    }
  ...
  private requestGlobalSearch(key: string, query: string): void {
    ...
    void this.plugin.searchKnowledgeBases(query, {
      ...
      this.globalSearchPendingKey = "";
      this.renderTree();
      const pendingReturn = this.pendingReturnScroll?.searchKey === key
        ? this.pendingReturnScroll
        : null;
      if (pendingReturn) {
        this.pendingReturnScroll = null;
        this.restoreReturnScrollPosition(pendingReturn.listScrollTop, pendingReturn.detailScrollTop);
      } else {
        this.resetSearchScrollPosition();
      }
    }).catch((error) => {
      ...
      if (this.pendingReturnScroll?.searchKey === key) this.pendingReturnScroll = null;
  ...
  public openPlaceholderResolutionQueue(): void {
    if (!this.guardLoadedBase()) return;
    const queue = this.unresolvedPlaceholderQueue();
    this.query = "";
    this.parsedQuery = parseQuery("");
    this.collapsedQueues.delete("imported-placeholders");
    this.plugin.data.collapsed.queues = [...this.collapsedQueues];
    this.run(async () => {
      if (this.plugin.data.activeTab !== "queues") await this.changeTab("queues", true);
      else {
        await this.plugin.saveViewState();
        if (this.guardLoadedBase()) this.render();
      }
      if (queue.total === 0) new Notice("No imported placeholders need a Markdown note in this knowledge base.");
    });
  }

  public startResolveNextPlaceholder(): void {
    if (!this.guardLoadedBase()) return;
    const next = this.unresolvedPlaceholderQueue().ordered[0];
    if (!next) {
      new Notice("No imported placeholders need a Markdown note in this knowledge base.");
      return;
    }
    this.openPlaceholderActions(next);
  }
  ...
  public openAddActions(): void {
    ...
    const actions = [
      ...
      ...(!this.plugin.isClinicalMode() ? [{ id: "vault-note", title: "Link an existing vault note to a collection", description: "Add a Markdown note to a collection without moving or modifying the note.", icon: "file-plus-2" }] : []),
    ];
  ...
  private libraryCreatePlacementIssue(libraryId: string, target: CatalogPlacementTarget): string | null {
    if (typeof this.plugin.isDataReadOnly === "function" && this.plugin.isDataReadOnly()) {
      return "Creating notes is unavailable while organization data is read-only.";
    }
    const library = this.plugin.getLibrary(libraryId);
    if (!library || library.archivedAt !== null) return "That library is no longer available.";
    if (this.plugin.isClinicalMode() && library.sourceKind !== null) {
      return `The built-in ${library.name} library follows native clinical source classification.`;
    }
    ...
    const heading = target.headingId
      ...
      : target.subheadingId
        ? layout.find((candidate) => subheadingChain(candidate, target.subheadingId ?? "") !== null)
        : undefined;
    if (target.headingId && !heading) return "That library heading is no longer available. Reopen the action and try again.";
    if (target.subheadingId && (!heading || !subheadingChain(heading, target.subheadingId))) {
      return "That library subheading is no longer available. Reopen the action and try again.";
    }
    return null;
  }
  ...
  private startCreateLibraryNote(libraryId: string, target: CatalogPlacementTarget = {}): void {
    ...
    const placementIssue = (): string | null => {
      if (!ownsBase()) return "The active knowledge base changed. Close and reopen this form.";
      return this.libraryCreatePlacementIssue(libraryId, target);
    };
    const initialPlacementIssue = placementIssue();
    if (initialPlacementIssue) {
      new Notice(initialPlacementIssue);
      return;
    }
    const profile = this.plugin.getEffectiveLibraryNoteProfile(libraryId);
    const creationPolicy = {
      openedData: this.plugin.data,
      openedBaseId: this.plugin.getActiveKnowledgeBaseId(),
      openedDataEpoch: this.plugin.getDataEpoch(),
      openedExternalChangeGeneration: this.plugin.getExternalChangeGeneration?.() ?? 0,
    };
    const targetLabel = target.headingId || target.subheadingId
      ? collectionTargets(this.libraryLayout(libraryId))
        .find((candidate) => (!target.headingId || candidate.headingId === target.headingId)
          && candidate.subheadingId === target.subheadingId)?.label
      : undefined;
    const placementLabel = targetLabel ? `${library.name} / ${targetLabel}` : library.name;
    this.startCreateKnowledgeNote({
      folder: profile.folder,
      mode: profile.mode,
      templatePath: profile.templatePath,
    }, false, async () => { /* The combined creator already committed exact placement. */ }, `${library.singularName} created${targetLabel ? ` under ${placementLabel}` : ` and added to ${library.name}`}. The Markdown note was not rewritten.`, {
      createLabel: library.singularName,
      contextNotice: target.headingId || target.subheadingId
        ? `This Markdown note will be classified under ${placementLabel} after creation. This destination is fixed for this action.`
      ...
      createNote: (value) => this.plugin.createKnowledgeNoteInLibrary(value, libraryId, target, creationPolicy),
      placementIssue,
    });
  }
  ...
  private openPortableSubjectLinkPicker(record: VaultRecord): void {
    ...
    const subject = this.plugin.getPortableSubject(subjectId);
    if (!subject) {
      new Notice("This portable subject no longer exists.");
      return;
    }
    ...
    const configuredIdKey = subject.configuredId ? localeInvariantTaxonomyKey(subject.configuredId) : "";
    const ownerByPath = new Map(Object.entries(this.plugin.data.portableIndex.resolvedPathBySubjectId)
      .filter(([candidateId]) => candidateId !== subjectId)
      .map(([candidateId, path]) => [path, candidateId] as const));
    const matchNoteByPath = new Map(this.placeholderMatchNotes(files).map((candidate) => [candidate.path, candidate]));
    const candidateByPath = new Map<string, { rank: number; label: string }>();
    for (const file of files) {
      const candidate = matchNoteByPath.get(file.path);
      if (!candidate) continue;
      const titleMatches = localeInvariantTaxonomyKey(candidate.title) === titleKey;
      const idMatches = Boolean(configuredIdKey
        && candidate.curriculumId
        && localeInvariantTaxonomyKey(candidate.curriculumId) === configuredIdKey);
      if (!titleMatches && !idMatches) continue;
      const owner = ownerByPath.get(file.path);
      const kind = titleMatches && idMatches
        ? { rank: 0, label: `Exact title + configured ID candidate (${subject.title} · ${subject.configuredId})` }
        : idMatches
          ? { rank: 2, label: `Exact configured ID candidate (${subject.configuredId})` }
          : { rank: 4, label: `Exact title candidate (${subject.title})` };
      candidateByPath.set(file.path, {
        rank: kind.rank + (owner ? 1 : 0),
        label: owner ? `${kind.label} — existing portable identity` : `${kind.label} — review before linking`,
      });
    }
    files.sort((left, right) => (candidateByPath.get(left.path)?.rank ?? 10)
      - (candidateByPath.get(right.path)?.rank ?? 10)
    ...
    new VaultFilePickerModal(
      ...
      (file) => candidateByPath.get(file.path)?.label ?? "",
    ).open();
  }
  ...
  private openPlaceholderActions(record: VaultRecord): void {
    if (!this.guardLoadedBase()) return;
    if (typeof this.plugin.isDataReadOnly === "function" && this.plugin.isDataReadOnly()) {
      new Notice("Creating or linking a placeholder is unavailable while organization data is read-only.", 8000);
      return;
    }
    const ownsBase = this.createOpenedBaseGuard();
  ...
  private openMembershipExplanation(record: VaultRecord): void {
    if (!this.guardLoadedBase()) return;
    new MembershipExplanationModal(this.app, record, this.plugin.data).open();
  }
  ...
  public startCreateKnowledgeNote(
    ...
    formContext?: CreateKnowledgeNoteFormContext,
  ): void {
    if (!this.guardLoadedBase()) return;
    const ownsBase = this.createOpenedBaseGuard();
    const settings = this.plugin.data.settings;
    ...
    const { placementIssue, createNote, ...modalContext } = formContext ?? {};
    new KnowledgeNoteModal(this.app, {
      itemSingular: settings.itemSingular,
      ...modalContext,
      templates: this.plugin.getTemplateFiles(),
      ...
      validate: (value) => {
        if (!ownsBase()) return "The active knowledge base changed. Close and reopen this form.";
        const currentPlacementIssue = placementIssue?.();
        if (currentPlacementIssue) return currentPlacementIssue;
        if (!onCreated) {
      ...
      onSubmit: async (value) => {
        if (!ownsBase()) return;
        const currentPlacementIssue = placementIssue?.();
        if (currentPlacementIssue) throw new Error(currentPlacementIssue);
        const file = createNote
          ? await createNote(value)
          : await this.plugin.createKnowledgeNote(value, modalContext.tokenContext);
        if (!ownsBase()) {
  ...
  private startCollectionNote(target: Membership, create: boolean): void {
    if (!this.guardLoadedBase()) return;
    const ownsBase = this.createOpenedBaseGuard();
    const currentTarget = (): CollectionTarget | undefined => collectionTargets(this.plugin.data.collections)
      .find((candidate) => candidate.headingId === target.headingId && candidate.subheadingId === target.subheadingId);
    const placementIssue = (): string | null => {
      if (!ownsBase()) return "The active knowledge base changed. Close and reopen this action.";
      if (this.plugin.isDataReadOnly()) return "Organization is read-only.";
      return currentTarget() ? null : "That collection destination no longer exists. Reopen the action.";
    };
    const label = currentTarget()?.label;
    const issue = placementIssue();
    if (issue || !label) { new Notice(issue ?? "That collection destination no longer exists."); return; }
    const place = async (file: TFile): Promise<void> => {
      const assertCurrentFile = (): void => {
        if (file.extension.toLocaleLowerCase() !== "md" || this.app.vault.getAbstractFileByPath(file.path) !== file) {
          throw new Error("That Markdown note is no longer available. Reopen the action and choose it again.");
        }
      };
      const issue = placementIssue();
      if (issue) throw new Error(issue);
      assertCurrentFile();
      await this.plugin.mutate("Add note to collection", () => {
        const issue = placementIssue();
        if (issue) throw new Error(issue);
        assertCurrentFile();
        if (!this.addMembership(file.path, target)) throw new Error("That collection destination no longer exists.");
      });
    };
    if (create) {
      this.startCreateKnowledgeNote({}, false, place, `Note created and added to ${label}.`, {
        contextNotice: `This note will be added to ${label}. The collection destination is fixed for this action.`,
        hideCollectionToggle: true,
        placementIssue,
      });
    } else {
      new VaultFilePickerModal(this.app, this.plugin.getVaultNoteFiles(false), `Add existing note to ${label}`, async (file) => {
        if (!ownsBase()) return;
        await place(file);
        if (ownsBase()) new Notice(`Added to ${label}. The source note stayed in place.`);
      }).open();
    }
  }
  ...
  private render(preserveBrowseLimits = false): void {
    ...
    this.contentEl.toggleClass("is-density-compact", this.viewDensity === "compact");
    ...
    const primaryActions = header.createDiv({ cls: "ent-cc-header-actions" });
    const readOnly = typeof this.plugin.isDataReadOnly === "function" && this.plugin.isDataReadOnly();
    if (!this.plugin.data.settings.setupComplete) {
      const setup = primaryActions.createEl("button", { cls: "ent-cc-button ent-cc-add-button", type: "button" });
      setIcon(setup.createSpan(), "settings-2");
      setup.createSpan({ text: "Set up" });
      disableWhenReadOnly(setup, readOnly, "Set up");
      setup.addEventListener("click", () => this.openSetupWizard());
    }
    const globalAdd = primaryActions.createEl("button", { cls: "ent-cc-button ent-cc-add-button ent-cc-primary-button ent-cc-main-add", type: "button" });
    setIcon(globalAdd.createSpan(), "plus");
    globalAdd.createSpan({ text: "Add" });
    disableWhenReadOnly(globalAdd, readOnly, "Add");
    globalAdd.addEventListener("click", () => this.openAddActions());
    const organize = primaryActions.createEl("button", {
    ...
    if (this.plugin.data.activeTab === "curriculum") {
      const manage = actions.createEl("button", { cls: "ent-cc-button", type: "button" });
      ...
      disableWhenReadOnly(arrange, readOnly, "Arrange Index");
      arrange.addEventListener("click", () => {
        this.curriculumArrangeMode = !this.curriculumArrangeMode;
        this.render(true);
        if (!this.curriculumArrangeMode) this.contentEl.querySelector<HTMLElement>(".ent-cc-workspace-options summary")?.focus({ preventScroll: true });
      });
    }
    if (this.plugin.data.activeTab === "collections") {
      ...
      edit.createSpan({ text: this.editMode ? "Done" : "Edit" });
      disableWhenReadOnly(edit, readOnly, "Edit collections");
      edit.addEventListener("click", () => {
        this.editMode = !this.editMode;
        this.render(true);
        if (!this.editMode) this.contentEl.querySelector<HTMLElement>(".ent-cc-workspace-options summary")?.focus({ preventScroll: true });
      });
    }
    const activeLibraryId = libraryIdForTab(this.plugin.data.activeTab);
    const activeLibrary = activeLibraryId ? this.plugin.getLibrary(activeLibraryId) : null;
    if (activeLibraryId && activeLibrary) {
      const addHeading = actions.createEl("button", { cls: "ent-cc-button", type: "button" });
      ...
      disableWhenReadOnly(edit, readOnly, "Arrange Library records");
      edit.addEventListener("click", () => {
        if (this.editMode) {
          this.editMode = false;
          this.render(true);
          this.contentEl.querySelector<HTMLElement>(".ent-cc-workspace-options summary")?.focus({ preventScroll: true });
          return;
        }
        const ownsBase = this.createOpenedBaseGuard();
        this.run(async () => {
    ...
    const density = actions.createEl("button", {
      cls: "ent-cc-button ent-cc-density-toggle", type: "button",
      text: this.viewDensity === "compact" ? "Comfortable" : "Compact",
      attr: { "aria-label": this.viewDensity === "compact" ? "Use comfortable view density" : "Use compact view density", "aria-pressed": String(this.viewDensity === "compact"), "data-kbcc-focus": "density" },
    });
    density.addEventListener("click", () => {
      this.viewDensity = this.viewDensity === "compact" ? "comfortable" : "compact";
      const focus = captureViewFocus(this.contentEl);
      const scroll = this.paneLayout === "wide" ? this.treeEl?.scrollTop ?? 0 : this.workspaceEl?.scrollTop ?? 0;
      this.render(true);
      this.restoreReturnScrollPosition(scroll, 0);
    ...
    this.treeEl = workspace.createDiv({
      ...
      attr: {
        id: panelId,
        role: "tabpanel",
        "aria-labelledby": this.tabElementId(this.plugin.data.activeTab),
        "aria-describedby": `ent-cc-record-shortcuts-${this.viewInstanceId}`,
        tabindex: "0",
      },
    });
    this.renderTree();
  ...
  private renderTabs(parent: HTMLElement): void {
    ...
    for (const tab of tabs) {
      ...
      this.updateTabCountBadge(button.createSpan({ cls: "ent-cc-tab-count" }), tab.id);
  ...
  /** Capture the exact visible route before this view opens a Markdown note. */
  public captureReturnViewState(): KbccReturnViewState {
    const compact = this.paneLayout !== "wide";
    const inspectorBody = this.inspectorEl?.querySelector<HTMLElement>(".ent-cc-inspector-body");
    const selectedPath = this.plugin.data.selectedPath;
    return {
      activeTab: this.plugin.data.activeTab,
      selectedPath,
      query: this.query,
      ...this.currentSearchFilters(),
      // A wide inspector is already visible beside the list. Preserve that
      // semantic detail route so a later compact return opens the same record.
      detailVisible: Boolean(selectedPath) && Boolean(compact ? this.mobileInspectorOpen : this.inspectorSelectedByUser),
      browseRowLimit: Math.min(MAX_KBCC_RETURN_BROWSE_LIMIT, Math.max(1, this.browseRowLimit)),
      browseStructureLimit: Math.min(MAX_KBCC_RETURN_BROWSE_LIMIT, Math.max(1, this.browseStructureLimit)),
      listScrollTop: Math.max(0, compact
        ? this.workspaceEl?.scrollTop ?? this.mobileTreeScrollTop
        : this.treeEl?.scrollTop ?? this.mobileTreeScrollTop),
      detailScrollTop: Math.max(0, compact
        ? inspectorBody?.scrollTop ?? this.mobileInspectorScrollTop
        : this.inspectorEl?.scrollTop ?? this.mobileInspectorScrollTop),
    };
  }

  /** Restore a note-bound route after its knowledge base has been activated. */
  public async restoreReturnViewState(state: KbccReturnViewState): Promise<boolean> {
    if (!this.guardLoadedBase()) return false;
    const availableTabs = tabDefinitions(this.plugin.data.settings, this.records, this.plugin.getLibraries());
    if (!availableTabs.some((tab) => tab.id === state.activeTab)) return false;
    this.cancelPendingGlobalSearch();
    this.globalSearchResult = null;
    this.globalSearchResultKey = "";
    this.globalSearchResultScopeKey = "";
    this.query = state.query;
    this.restoreSearchFilters(state, state.activeTab);
    this.parsedQuery = parseQuery(state.query);
    this.plugin.data.activeTab = state.activeTab;
    this.plugin.data.selectedPath = state.selectedPath && this.recordByPath.has(state.selectedPath)
      ? state.selectedPath
      : "";
    this.browseRowLimit = Math.min(MAX_KBCC_RETURN_BROWSE_LIMIT, Math.max(1, state.browseRowLimit));
    this.browseStructureLimit = Math.min(MAX_KBCC_RETURN_BROWSE_LIMIT, Math.max(1, state.browseStructureLimit));
    this.mobileTreeScrollTop = state.listScrollTop;
    this.mobileInspectorScrollTop = state.detailScrollTop;
    this.mobileInspectorOpen = state.detailVisible && Boolean(this.plugin.data.selectedPath);
    this.inspectorSelectedByUser = this.mobileInspectorOpen;
    this.mobileInspectorNeedsFocus = false;
    if (!await this.persistReturnedViewStateBestEffort()) return false;
    this.pendingReturnScroll = this.hasGlobalSearch()
      ? {
          searchKey: this.globalSearchKey(),
          listScrollTop: state.listScrollTop,
          detailScrollTop: state.detailScrollTop,
        }
      : null;
    this.render(true);
    this.restoreReturnScrollPosition(state.listScrollTop, state.detailScrollTop);
    this.focusReturnDestination(state.activeTab);
    return true;
  }

  /** Open a clean Command Center home when the note has no matching origin. */
  public async openHomePage(): Promise<void> {
    if (!this.guardLoadedBase()) return;
    const availableTabs = tabDefinitions(this.plugin.data.settings, this.records, this.plugin.getLibraries());
    const homeTab = availableTabs.some((tab) => tab.id === this.plugin.data.settings.defaultTab)
      ? this.plugin.data.settings.defaultTab
      : "curriculum";
    this.cancelPendingGlobalSearch();
    this.globalSearchResult = null;
    this.globalSearchResultKey = "";
    ...
    this.mobileTreeScrollTop = 0;
    this.mobileInspectorScrollTop = 0;
    if (!await this.persistReturnedViewStateBestEffort()) return;
    this.render();
    this.restoreReturnScrollPosition(0, 0);
    this.focusReturnDestination(homeTab);
  }

  /** A device-local write failure must not prevent an explicit in-session navigation. */
  private async persistReturnedViewStateBestEffort(): Promise<boolean> {
    if (this.viewClosed) return false;
    let persistenceError: unknown = null;
    await this.plugin.saveViewState(false, (error) => { persistenceError = error; });
    // A base/epoch change is a real stale-view boundary, not a persistence
    // failure to ignore. Only render when this view still owns the live data.
    if (this.viewClosed || !this.guardLoadedBase()) return false;
    if (persistenceError) {
      console.warn("Knowledge Base Command Center could not persist returned view state", persistenceError);
      new Notice("The command center page opened, but this device could not preserve its current view state for a restart.", 8000);
    }
    return true;
  }

  /** Explicit editor-header navigation must establish focus inside the revealed view. */
  private focusReturnDestination(tab: MainTab): void {
    const focus = (): void => {
      if (this.viewClosed) return;
      const compactInspector = this.paneLayout !== "wide" && this.mobileInspectorOpen;
      const target = compactInspector
        ? this.inspectorEl?.querySelector<HTMLElement>(".ent-cc-inspector-close")
        : this.contentEl.querySelector<HTMLElement>(`[data-tab="${tab}"]`);
      target?.focus({ preventScroll: true });
    };
    focus();
    this.timerWindow.requestAnimationFrame(focus);
  }
  ...
  private restoreReturnScrollPosition(listScrollTop: number, detailScrollTop: number): void {
    ...
    this.timerWindow.requestAnimationFrame(() => {
      apply();
      this.timerWindow.requestAnimationFrame(apply);
    });
  }
  ...
  private updateTabCountBadge(badge: HTMLElement, tab: MainTab): void {
    const count = this.tabCount(tab);
    const units = `${tab === "collections" ? "member " : ""}${count === 1 ? "entry" : "entries"}`;
    badge.setText(tab === "collections" ? `${count} ${count === 1 ? "member" : "members"}` : String(count));
    badge.setAttribute("aria-label", `${count} ${units}`);
    badge.setAttribute("title", `${count} ${units}`);
  }
  ...
  private refreshChromeCounts(): void {
    ...
    this.contentEl.querySelectorAll<HTMLElement>(".ent-cc-tab").forEach((tab) => {
      ...
      const badge = tab.querySelector<HTMLElement>(".ent-cc-tab-count");
      if (badge) this.updateTabCountBadge(badge, id);
    });
  }
  ...
  private renderSearch(parent: HTMLElement): void {
    ...
    const input = box.createEl("input", {
      ...
      attr: {
        id: `ent-cc-search-${this.viewInstanceId}`,
    ...
    disableWhenReadOnly(save, readOnly, "Save this search");
    ...
    bulkButton.disabled = readOnly || !this.hasGlobalSearch();
    disableWhenReadOnly(bulkButton, readOnly, "Add search results to a collection");
    bulkButton.addEventListener("click", () => this.addMatchingRecordsToCollection());
    const saved = searchRow.createEl("button", { cls: "ent-cc-button ent-cc-saved-button", type: "button" });
    ...
    for (const [value, label] of [["all", "All entries"], ["linked", "Linked notes"], ["placeholders", "No note"]]) availability.createEl("option", { value, text: label });
    availability.value = this.searchAvailability;
    const linkedLabel = options.createEl("label", { cls: "ent-cc-search-option ent-cc-linked-first" });
    ...
    if (this.plugin.isClinicalMode()) {
      ...
      for (const [token, label] of definitions) {
        ...
        const chip = chips.createEl("button", {
          cls: `ent-cc-filter-chip ${active ? "is-active" : ""}`,
          type: "button",
          text: label,
        ...
        chip.addEventListener("click", () => {
          ...
          const pressed = this.query.toLowerCase().split(/\s+/).includes(token.toLowerCase());
          chip.toggleClass("is-active", pressed);
  ...
  private updateCount(visible: number): void {
    ...
    if (this.plugin.data.activeTab === "queues") {
      ...
      const tab = this.plugin.data.activeTab;
      const paths = tab === "collections" ? new Set(collectionPaths(this.plugin.data.collections)) : null;
      const records = this.recordsForActiveTab().filter((record) => !paths || paths.has(record.path));
      const collectionCount = this.plugin.data.collections.length;
      this.countEl.createSpan({ text: `${paths ? `${collectionCount} ${collectionCount === 1 ? "collection" : "collections"} · ` : ""}${recordAvailabilitySummary(records)}` });
    }
  ...
  private hasGlobalSearch(): boolean {
    return Boolean(this.query?.trim()) || (this.searchScope ?? "all") !== "all"
      || (this.searchAvailability ?? "all") !== "all" || Boolean(this.searchLinkedFirst);
  }
  ...
  private renderTree(): void {
    ...
    this.treeEl.createDiv({
      ...
      attr: { id: `ent-cc-record-shortcuts-${this.viewInstanceId}` },
    });
    ...
    let visible = 0;
    const tab = this.plugin.data.activeTab;
    const searching = this.hasGlobalSearch();
    if (!searching) {
    ...
    if (!searching && (this.browseRowsOmitted > 0 || this.browseStructuresOmitted > 0)) {
      this.renderBrowseLimit(body);
    }
    const globalSearchPending = Boolean(searching && this.globalSearchPendingKey === this.globalSearchKey());
    const globalSearchFailed = Boolean(searching && this.globalSearchErrorKey === this.globalSearchKey());
    if (visible === 0
      && !globalSearchPending
      && !globalSearchFailed
      && !(!searching && tab === "collections")
      && !(!searching && tab === "inbox")
  ...
  private renderBrowseLimit(parent: HTMLElement): void {
    ...
    const showMore = message.createEl("button", {
      cls: "ent-cc-button",
      attr: { "data-kbcc-focus": "browse-more" },
      text: nextRecords > 0
        ? `Show ${nextRecords.toLocaleString()} more`
        : `Show ${nextStructures.toLocaleString()} more sections`,
    });
    showMore.addEventListener("click", () => {
      const previousRows = this.browseRowsRendered;
      this.browseRowLimit += MAX_RENDERED_BROWSE_RECORDS;
      this.browseStructureLimit += MAX_RENDERED_BROWSE_STRUCTURES;
      this.renderTree();
      if (!this.treeEl?.querySelector('[data-kbcc-focus="browse-more"]')) {
        (this.treeEl?.querySelectorAll<HTMLElement>(".ent-cc-subject-title")[previousRows] ?? this.treeEl)?.focus({ preventScroll: true });
      }
    });
  }
  ...
  private renderCurriculumDomain(parent: HTMLElement, domain: CurriculumDomainTree, roots: CurriculumTreeNode[]): void {
    ...
    title.setAttribute("data-kbcc-focus", `domain:${domain.domain}`);
  ...
  private renderCurriculumNode(parent: HTMLElement, node: CurriculumTreeNode, depth: number): void {
    ...
    const selected = this.plugin.data.selectedPath === record.path;
    const section = parent.createDiv({ cls: "ent-cc-curriculum-node" });
    const row = section.createDiv({
      cls: `ent-cc-row ent-cc-subject-row ent-cc-curriculum-row ${record.isPlaceholder ? "ent-cc-placeholder-row" : ""} ${selected ? "is-selected" : ""}`,
      attr: { "data-record-path": record.path, "data-source-active": "true" },
    });
    row.addClass(`ent-cc-depth-${Math.min(depth, 12)}`);
    if (node.children.length > 0) {
      const disclosure = disclosureButton(row, collapsed, record.title, record.path);
      disclosure.addEventListener("click", () => {
    ...
    const title = row.createEl("button", {
      ...
      attr: {
        type: "button",
        dir: "auto",
        "aria-keyshortcuts": RECORD_KEYBOARD_SHORTCUTS,
        "data-kbcc-focus": `record:${record.path}`,
        "aria-label": `${record.title}, ${this.recordRoleName(record)}.${provenance ? ` ${provenance.ariaLabel}.` : ""}`,
      },
    });
    ...
    this.renderIndexRowMetadata(
      row,
      record.isPlaceholder ? PLACEHOLDER_STATUS_LABEL : record.curriculumId || (this.plugin.isClinicalMode() ? "supporting" : this.plugin.data.settings.itemSingular),
      provenance,
    );
    const badges = row.createDiv({ cls: "ent-cc-row-badges" });
    if (record.isPlaceholder) this.renderPlaceholderBadge(badges, provenance);
    else if (provenance) this.renderCompactIndexMembershipBadge(badges, provenance);
    if (this.hasCurriculumVisualPlacement(record.path)) {
      statusIconBadge(badges, "ent-cc-visual-badge", "move", "Custom visual placement");
    }
    ...
    if (this.plugin.isClinicalMode() && this.plugin.data.settings.showSafetyBadges && record.safetyCritical) {
      statusIconBadge(badges, "ent-cc-safety-badge", "shield-alert", "Safety-critical");
    }
    if (this.plugin.data.pinnedPaths.includes(record.path)) {
      statusIconBadge(badges, "ent-cc-pin-badge", "pin", "Pinned");
    }
  ...
  private renderLayoutNode(parent: HTMLElement, context: LayoutRenderContext, node: LayoutNode, depth: number): void {
    ...
    disclosureButton(row, collapsed, node.title, `${context.kind}:${context.heading.id}:${node.id}`).addEventListener("click", toggle);
    const leading = row.createSpan({ cls: "ent-cc-leading-icon" });
    ...
    title.setAttribute("data-kbcc-focus", `heading:${context.kind}:${context.heading.id}:${node.id}`);
    ...
    if (!isLibrary && layoutSubjects(node).length === 0 && childSubheadings(node).length === 0) {
      const empty = content.createDiv({ cls: "ent-cc-empty-collection" });
      empty.createSpan({ text: "No notes in this collection yet." });
      for (const [label, create] of [["Add existing note", false], ["Create note", true]] as const) {
        const add = empty.createEl("button", { cls: "ent-cc-button", text: label, type: "button" });
        disableWhenReadOnly(add, this.plugin.isDataReadOnly(), `${label} in this collection`);
        add.addEventListener("click", () => this.startCollectionNote(membership, create));
      }
    }
  ...
  private renderQueue(parent: HTMLElement, queue: QueueDefinition): number {
    ...
    const leading = row.createSpan({ cls: "ent-cc-leading-icon" });
    setIcon(leading, queue.id === "next" ? "list-checks"
      : queue.id === "pinned" ? "pin"
        : queue.id === "imported-placeholders" ? "file-question"
          : "sparkles");
    const title = row.createEl("button", { cls: "ent-cc-row-title", text: queue.title, attr: { title: queue.description, dir: "auto" } });
    title.setAttribute("data-kbcc-focus", `queue:${queue.id}`);
    title.addEventListener("click", () => {
      if (collapsed) this.collapsedQueues.delete(queue.id); else this.collapsedQueues.add(queue.id);
      this.persistCollapseState();
      this.renderTree();
  ...
  private renderGlobalSearchResultSet(
    ...
    const otherBases = (results.counts ?? []).filter(({ total, source }) => total > 0
      && !results.groups.some((group) => group.source.baseId === source.baseId));
    for (const { source, total } of otherBases) {
      parent.createDiv({ cls: "ent-cc-search-other-base", text: `${source.baseName}: ${total.toLocaleString()} matching entries beyond this page. Switch to this base and choose This base to narrow the search.`, attr: { role: "status" } });
    }
  ...
  private recordRoleName(record: VaultRecord, data = this.plugin.data): string {
    if (record.isPlaceholder) return PLACEHOLDER_STATUS_LABEL;
    if (record.libraryId) {
  ...
  /** Compact panes replace the hidden metadata column with one canonical state badge. */
  private renderPlaceholderBadge(parent: HTMLElement, provenance: IndexMembershipProvenance | null): void {
    const accessibleLabel = `${PLACEHOLDER_STATUS_LABEL}.${provenance ? ` ${provenance.ariaLabel}.` : ""}`;
    parent.createSpan({
      cls: "ent-cc-placeholder-badge",
      text: PLACEHOLDER_STATUS_LABEL,
      attr: { title: accessibleLabel, "aria-label": accessibleLabel },
    });
  }
  ...
  private renderIndexRowMetadata(
    ...
    const metadata = row.createSpan({
      cls: `ent-cc-subject-id ${provenance && provenance.kind !== "imported-placeholder" ? "has-membership-provenance" : ""}`,
      attr: { dir: "auto", ...(provenance ? { title: provenance.ariaLabel } : {}) },
    });
    metadata.createSpan({ cls: "ent-cc-subject-id-value", text });
    if (provenance && provenance.kind !== "imported-placeholder") {
      metadata.createSpan({
  ...
  private renderRecordRow(
    ...
    const title = row.createEl("button", {
      ...
      attr: {
        ...
        dir: "auto",
        "aria-keyshortcuts": RECORD_KEYBOARD_SHORTCUTS,
        "aria-label": `${record.title}, ${this.recordRoleName(record, sourceData)}${source ? ` in ${source.baseName}` : ""}.${provenance ? ` ${provenance.ariaLabel}.` : ""}`,
        "data-kbcc-focus": `record:${source?.baseId ?? this.plugin.getActiveKnowledgeBaseId()}:${record.path}`,
      },
    });
    if (selected) title.setAttribute("aria-current", "true");
    title.addEventListener("click", () => {
    ...
    this.renderIndexRowMetadata(
      row,
      record.isPlaceholder ? PLACEHOLDER_STATUS_LABEL : record.curriculumId || (sourceData.settings.workspaceMode === "ent-clinical"
  ...
  private selectRecord(path: string): void {
    ...
    this.inspectorSelectedByUser = true;
    const compact = this.isCompactInspectorLayout();
    if (compact) {
  ...
  /** Update transient desktop selection in place so the focused row is never detached. */
  private syncSelectedRecordRows(path: string): void {
    this.treeEl?.querySelectorAll<HTMLElement>(".ent-cc-subject-row[data-record-path]").forEach((row) => {
      const selected = row.getAttribute("data-source-active") !== "false"
        && row.getAttribute("data-record-path") === path;
      row.toggleClass("is-selected", selected);
      const title = row.querySelector<HTMLElement>(".ent-cc-subject-title");
      if (!title) return;
      if (selected) title.setAttribute("aria-current", "true");
  ...
  private renderInspector(): void {
    ...
    if (!record.isPlaceholder) body.createDiv({ cls: "ent-cc-updated-label", text: relativeUpdatedTime(record.mtime), attr: record.mtime > 0 ? { title: new Date(record.mtime).toLocaleString() } : {} });
    ...
    if (record.isPlaceholder) {
      ...
      disableWhenReadOnly(complete, readOnly, "Create or link a placeholder note");
      ...
      disableWhenReadOnly(add, readOnly, "Add to a collection");
      add.addEventListener("click", () => this.openCollectionPicker(record.path));
      const why = actions.createEl("button", { cls: "ent-cc-button", type: "button" });
      setIcon(why.createSpan(), "circle-help"); why.createSpan({ text: "Why this appears" });
      why.addEventListener("click", () => this.openMembershipExplanation(record));
      const pin = iconButton(actions, this.plugin.data.pinnedPaths.includes(record.path) ? "pin-off" : "pin", this.plugin.data.pinnedPaths.includes(record.path) ? "Unpin" : "Pin");
      disableWhenReadOnly(pin, readOnly, "Update pin status");
      pin.addEventListener("click", () => this.run(() => this.togglePin(record.path)));
    ...
    if (this.plugin.isClinicalMode() || record.reviewStatus || record.safetyCritical) {
      ...
      if (this.plugin.isClinicalMode() || record.reviewStatus) statusLine.createSpan({
        text: record.reviewStatus ? readableStatus(record.reviewStatus) : "Review metadata missing",
    ...
    if (canRelinkPortableRecord(record)) {
      ...
      disableWhenReadOnly(changeLink, readOnly, "Change a portable note link");
      ...
      disableWhenReadOnly(unlink, readOnly, "Unlink a portable note");
    ...
    if (!record.libraryId
      ...
      disableWhenReadOnly(moveGroup, readOnly, "Move visual group placement");
    ...
    if (this.plugin.isClinicalMode() && record.role === "proposal") {
      ...
      promote.disabled = record.aiLock || readOnly;
      disableWhenReadOnly(promote, readOnly, "Promote a proposal");
    ...
    if (this.plugin.isClinicalMode() && record.role === "canonical" && this.plugin.data.settings.enableAdvancedCanonicalActions) {
      ...
      placement.disabled = record.aiLock || readOnly;
      disableWhenReadOnly(placement, readOnly, "Edit canonical placement");
  ...
  private renderStudyActions(parent: HTMLElement, record: VaultRecord): void {
    ...
    const isNext = this.plugin.data.nextStudyPaths.includes(record.path);
    const next = section.createEl("button", {
      cls: `ent-cc-study-action ${isNext ? "is-active" : ""}`,
      type: "button",
      attr: { "aria-pressed": String(isNext) },
    });
    setIcon(next.createSpan(), isNext ? "check" : "list-plus");
    next.createSpan({ text: isNext
      ? `Remove from ${this.plugin.isClinicalMode() ? "Next to study" : "Next list"}`
      : `Add to ${this.plugin.isClinicalMode() ? "Next to study" : "Next list"}` });
    disableWhenReadOnly(next, this.plugin.isDataReadOnly(), "Update the Next list");
  ...
  private unresolvedPlaceholderQueue(): {
    records: VaultRecord[];
    ordered: VaultRecord[];
    total: number;
    withCandidates: number;
  } {
    // Prototype-only unit fixtures from older boundaries may not provide the
    // eligible-file method. Production always does; retain record-only behavior
    // for those fixtures instead of weakening the runtime candidate universe.
    const boundary = this.plugin as unknown as {
      getActiveKnowledgeBaseId?: () => string;
      getVaultNoteGeneration?: () => number;
      getVaultNoteFiles?: (includeRestricted?: boolean) => TFile[];
    };
    const baseId = boundary.getActiveKnowledgeBaseId?.call(this.plugin) ?? "fixture-base";
    const candidateGeneration = boundary.getVaultNoteGeneration?.call(this.plugin) ?? -1;
    const { workspaceMode, idProperty, templatesFolder } = this.plugin.data.settings;
    const candidateSettingsKey = `${workspaceMode}\0${idProperty}\0${templatesFolder}`;
    if (this.placeholderQueueCache?.records === this.records
      && this.placeholderQueueCache.baseId === baseId
      && this.placeholderQueueCache.candidateGeneration === candidateGeneration
      && this.placeholderQueueCache.candidateSettingsKey === candidateSettingsKey) return this.placeholderQueueCache;
    const files = boundary.getVaultNoteFiles ? boundary.getVaultNoteFiles.call(this.plugin, false) : null;
    const queue = buildPlaceholderResolutionQueue(
      this.plugin.data,
      files ? this.placeholderMatchNotes(files) : this.records,
      this.records,
    );
    const ordered = queue.items
      .map((item) => this.recordByPath.get(item.path))
      .filter((record): record is VaultRecord => Boolean(record));
    this.placeholderQueueCache = {
      records: this.records,
      baseId,
      candidateGeneration,
      candidateSettingsKey,
      ordered,
      total: queue.total,
      withCandidates: queue.withCandidates,
    };
    return this.placeholderQueueCache;
  }

  /** Candidate counts and picker highlighting share this complete eligible-note projection. */
  private placeholderMatchNotes(files: readonly TFile[]): PlaceholderMatchNote[] {
    return buildEligiblePlaceholderMatchNotes(
      files,
      this.records,
      this.plugin.data.settings,
  ...
  private smartQueues(): QueueDefinition[] {
    ...
    const placeholders = this.unresolvedPlaceholderQueue();
  ...
  private showLibraryHeadingMenu(event: MouseEvent, libraryId: string, heading: LayoutHeading): void {
    ...
    menu.addSeparator();
    if (!this.plugin.isClinicalMode() || library.sourceKind === null) {
      menu.addItem((item) => item.setTitle("Create note here…").setIcon("file-plus-2").setDisabled(readOnly).onClick(() => {
        if (!ownsBase()) return;
        const currentLibrary = this.plugin.getLibrary(libraryId);
        if (!currentLibrary || currentLibrary.archivedAt !== null) {
          new Notice("That library is no longer available.");
          return;
        }
        if (!layoutIsCurrent()) return;
        const currentHeading = this.libraryLayout(libraryId).find((candidate) => candidate.id === heading.id);
        if (!currentHeading) {
          new Notice("That library heading is no longer available. Reopen the action and try again.");
          return;
        }
        this.startCreateLibraryNote(libraryId, { headingId: currentHeading.id });
      }));
    }
  ...
  private showLibrarySubheadingMenu(event: MouseEvent, libraryId: string, heading: LayoutHeading, subheading: LayoutSubheading): void {
    ...
    menu.addSeparator();
    if (library && (!this.plugin.isClinicalMode() || library.sourceKind === null)) {
      menu.addItem((item) => item.setTitle("Create note here…").setIcon("file-plus-2").setDisabled(readOnly).onClick(() => {
        if (!ownsBase()) return;
        const currentLibrary = this.plugin.getLibrary(libraryId);
        if (!currentLibrary || currentLibrary.archivedAt !== null) {
        ...
        this.startCreateLibraryNote(libraryId, { headingId: heading.id, subheadingId: current.node.id });
      }));
    }
    menu.addItem((item) => item.setTitle("Place record here…").setIcon("file-input").onClick(() => {
  ...
  private showRecordMenu(event: MouseEvent, record: VaultRecord, membership?: Membership, libraryMembership?: LibraryMembership): void {
    ...
    menu.addItem((item) => item.setTitle("Why this appears…").setIcon("circle-help").onClick(() => {
      if (ownsBase()) this.openMembershipExplanation(record);
    }));
    if (canRelinkPortableRecord(record)) {
  ...
  private showGlobalMenu(event: MouseEvent): void {
    ...
    const placeholders = this.unresolvedPlaceholderQueue();
    menu.addItem((item) => item
      .setTitle(`Open placeholder queue (${placeholders.total})`)
      .setIcon("file-question")
      .onClick(() => {
        if (ownsBase()) this.openPlaceholderResolutionQueue();
      }));
    menu.addItem((item) => item
      .setTitle("Resolve next placeholder…")
      .setIcon("list-start")
      .setDisabled(placeholders.total === 0)
      .onClick(() => {
        if (ownsBase()) this.startResolveNextPlaceholder();
      }));
    ...
    new TextPromptModal(this.app, {
      ...
      onSubmit: async (name) => {
        ...
        await this.plugin.mutate(`Save view “${name}”`, () => {
          this.plugin.data.savedViews.push({ id: makeId("view"), name, tab: this.plugin.data.activeTab, query: this.query, ...this.currentSearchFilters() });
        });
      },
    }).open();
  }
  ...
  private showSavedViews(event: MouseEvent): void {
    ...
    for (const view of this.plugin.data.savedViews) {
      menu.addItem((item) => item.setTitle(view.name).setIcon("bookmark").onClick(() => this.run(async () => {
        ...
        this.restoreSearchFilters(view, this.plugin.data.activeTab);
