...
test("public repository metadata is present", async () => {
  ...
  assert.ok(currentChangelog.trim().length > `## ${String(manifest.version)}`.length, "the current release must contain curated notes, not only a version heading");
  const released020Start = changelog.indexOf("## 0.20.0");
  assert.notEqual(released020Start, -1, "the 0.20.0 release record must remain in the changelog");
  const released020End = changelog.indexOf("\n## ", released020Start + 1);
  const released020Changelog = changelog.slice(released020Start, released020End < 0 ? undefined : released020End);
  assert.match(released020Changelog, /explicit search scope/iu);
  assert.match(released020Changelog, /pagination[^.]*keyboard focus[^.]*refresh/iu);
  assert.match(released020Changelog, /Settings[^.]*labels/iu);
  assert.match(released020Changelog, /import[- ]completion/iu);
  assert.match(released020Changelog, /Settings[^.]*spacing/iu);
  assert.match(released020Changelog, /unseen offline edits[^.]*conflict rescue/iu);
  const released0191Start = changelog.indexOf("## 0.19.1");
  assert.notEqual(released0191Start, -1, "the published 0.19.1 release record must remain in the changelog");
  const released0191End = changelog.indexOf("\n## ", released0191Start + 1);
  const released0191Changelog = changelog.slice(released0191Start, released0191End < 0 ? undefined : released0191End);
  assert.match(released0191Changelog, /false \*\*No linked note\*\* placeholders/iu);
  assert.match(released0191Changelog, /workspace layout ready/iu);
  assert.match(released0191Changelog, /initial metadata scan finishes/iu);
  assert.match(released0191Changelog, /does not change synced KBCC organization/iu);
  assert.match(released0191Changelog, /configured per device/iu);
